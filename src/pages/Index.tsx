...
const Index = () => {
  ...
  const getCurrentHintVideos = (): { url: string; label: string; text: string }[] => {
    ...
    if (activePhrase.id === "g1") {
      ...
      return [{ url: `${base}${key}-tip.webm`, label: selectedVariant === "adult" ? "สวัสดีผู้ใหญ่" : "สวัสดีเพื่อน", text: phraseHintMap[key] || "" }];
    }
    if (activePhrase.id === "g2") {
      const stepKey = byeStep === 1 ? "bye_me" : "bye_go";
      const stepLabel = byeStep === 1 ? "ฉัน" : "ไป";
      return [{ url: `${base}${stepKey}-tip.webm`, label: stepLabel, text: phraseHintMap[stepKey] || "" }];
    }
    if (activePhrase.id === "g3") {
      ...
      return [{ url: `${base}${stepKey}-tip.webm`, label: stepLabelFinal, text: phraseHintMap[stepKey] || "" }];
    }
    ...
    if (activePhrase.id === "g5") return [{ url: `${base}how_are_you-tip.webm`, label: "สบายดีไหม?", text: phraseHintMap["how_are_you"] || "" }];
    if (activePhrase.id === "g6") {
      const key = selectedVariant === "adult" ? "fine" : "unhappy";
      return [{ url: `${base}${key}-tip.webm`, label: selectedVariant === "adult" ? "สบายดี" : "ไม่สบายใจ", text: phraseHintMap[key] || "" }];
    }
    // Other phrases — map from modelClass
    const mc = effectivePhrase?.modelClass;
    if (mc) return [{ url: `${base}${mc}-tip.webm`, label: activePhrase.text, text: phraseHintMap[mc] || "" }];
    return [];
  };
