...
export function GameSidebar({
  ...
  return (
    <>
      {/* Overlay */}
      {isOpen && (
        <div className="fixed inset-0 bg-foreground/30 backdrop-blur-sm z-30 lg:hidden" onClick={onToggle} />
      )}

      {/* Sidebar wrapper — the arrow tab is positioned at the right edge of this wrapper */}
      <div
        className={`fixed lg:static z-40 top-0 left-0 h-dvh transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        style={{ width: "fit-content" }}
      >
        <aside
          className="h-full w-64 bg-primary border-r-[3px] border-foreground flex flex-col overflow-hidden rounded-r-3xl border-y-[3px] lg:rounded-none lg:border-y-0"
        >
